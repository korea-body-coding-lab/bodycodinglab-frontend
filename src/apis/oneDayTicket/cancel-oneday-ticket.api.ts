...
import { CANCEL_ONE_DAY_TICKET_URL } from "../constants";
...
export const cancelOneDayTicket = async (ticketId: number, dto: TicketCancelRequestDto, accessToken: string): Promise<ResponseDto<null>> => {
  try{
    const response = await axiosInstance.post(CANCEL_ONE_DAY_TICKET_URL(ticketId), dto, bearerAuthorization(accessToken));
