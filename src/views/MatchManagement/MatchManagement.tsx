...
function MatchManagement() {
  ...
  const [matchData, setMatchData] = useState<trainerMatchResponseDto | null>(
    null
  );
  const [waitingData, setWaitingData] =
    useState<trainerMatchWaitingListResponseDto | null>(null);
    ...
    const loadingTrainerData = async () => {
      ...
      try {
        ...
        if (matchResponse?.data) {
          ...
          const waitingResponse = await findMemberMatchWaitingListRequest(
            token
          );
