...
function ReadMemberMatch() {
  ...
  const [trainerData, setTrainerData] = useState<trainerMatchResponseDto | undefined>(undefined);
  ...
  return (
    <div css={m.MemberMatchContainerBox}>
      ...
      <div css={m.MemberMatchContainer}>
        ...
        <br />
        <div>
        <strong>매칭일 </strong><p>{new Date(trainerData.matchedAt).toLocaleString("ko-kR",{
          year: "numeric",
          month: "long",
          day:"numeric",
          hour:"2-digit",
        ...
        <div css={m.MemberMatchButtonContainer}>
          <button css={m.MatchButton} onClick={() => matchCancelButton(trainerData.matchId)}>매칭 취소</button>
        </div>
      </div>
    </div>
  )
